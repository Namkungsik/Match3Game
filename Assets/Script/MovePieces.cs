...
{
    public static MovePieces instance; // MovePieces 클래스의 싱글턴 인스턴스
    Match3 game; // 게임 로직을 다루는 객체

    NodePiece moving;    // 현재 움직이고 있는 NodePiece
    Point newIndex;      // 새로운 위치 인덱스
    Vector2 mouseStart;  // 마우스로 클릭한 시작 위치
    ...
    void Update()
    {
        ...
        {
            Vector2 dir = ((Vector2)Input.mousePosition - mouseStart); // 마우스 현재 위치와 시작 위치 간 차이 계산
            Vector2 nDir = dir.normalized; // 방향 벡터 정규화
            Vector2 aDir = new Vector2(Mathf.Abs(dir.x), Mathf.Abs(dir.y)); // 절대값 계산하여 x, y 이동 거리 비교
            ...
            {
                // x축 이동이 더 크면 좌우 이동, y축 이동이 더 크면 상하 이동으로 판정
                if (aDir.x > aDir.y)
                {
                    add = (new Point((nDir.x > 0) ? 1 : -1, 0)); // x축 이동 방향
                }
                else if (aDir.y > aDir.x)
                {
                    add = (new Point(0, (nDir.y > 0) ? -1 : 1)); // y축 이동 방향
                }
            }
            ...
            {
                pos += Point.mult(new Point(add.x, -add.y), 16).ToVector(); // 이동 방향에 따라 위치 보정
            }
            moving.MovePositionTo(pos); // 움직이는 조각의 위치 업데이트
        }
    }
    ...
    {
        if (moving != null || piece.value == -1) return; // 이미 움직이고 있거나, 빈 조각(-1)이면 무시
    ...
    {
        if (moving == null) return; // 움직이는 조각이 없으면 무시

        if (!newIndex.Equals(moving.index)) // 새로운 위치로 이동했다면
        {
